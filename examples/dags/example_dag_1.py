...
@dag(
    ...
    doc_md="""
        Example DAG showing how to use the `@task.pyspark_on_k8s` decorator. 
...
def example_dag_1():
    ...
    load_s3a_uri = load(s3a_uri=csv_s3a_uri, s3_bucket="{{ params.s3_bucket }}")
    transform(s3a_uri=load_s3a_uri, s3_bucket="{{ params.s3_bucket }}")
